# 네이버 데이터랩 쇼핑인사이트 오픈API로 키워드별 일별 성별 쇼핑 클릭 트렌드 수집
# 키워드당 1회 호출로 남/여 분해 결과를 받아 gender_m.csv, gender_f.csv 생성
...
with open(PROJECT_ROOT / "configs" / "collection.yaml", encoding="utf-8") as f:
    _cfg = yaml.safe_load(f)["click_gender"]
...
# 단일 키워드의 전체 기간 성별 쇼핑 클릭 트렌드 조회
# 반환: {group_code: {period: ratio, ...}, ...}
def fetch_keyword_gender(keyword):
    body = {
...
def main():
    ...
    for g in GROUPS:
        label = g["label"]
        path = OUTPUT_DIR / f"gender_{label}.csv"
        if path.exists():
    ...
    if not todo:
        return
    print(f"shopping_click_gender: {len(todo)}/{len(keywords)} keywords to collect")

    # 키워드별 성별 트렌드 수집 (중단 시 중간 결과 저장)
    rows = {g["label"]: {} for g in GROUPS}
    try:
        for i, kw in enumerate(todo, 1):
            grouped = fetch_keyword_gender(kw)
            for g in GROUPS:
