# 네이버 데이터랩 검색 트렌드 오픈API로 키워드별 일별 통합 검색량 트렌드 수집
...
OUTPUT_DIR = PROJECT_ROOT / "data" / "raw" / "search"
...
def main():
    ...
    if not todo:
        return
    print(f"search_trend: {len(todo)}/{len(keywords)} keywords to collect")

    # 키워드별 검색 트렌드 수집 (중단 시 중간 결과 저장)
    rows = {}
    try:
        for i, kw in enumerate(todo, 1):
            trend = fetch_search_trend(kw)
            rows[kw] = trend
